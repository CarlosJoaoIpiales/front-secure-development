...
export class AppTopBarComponent implements OnInit {
    ...
    loadUserCompanies(userId: string) {
        this.userCompanyService.getUserCompanies(userId).subscribe({
            next: (userCompanies) => {
                let companyIds = userCompanies
                    .map((uc: any) => uc.company_unique_key)
                    ...
                    next: (companies) => {
                        this.companies = [...companies
                            ...
                            .map((company: any): Company => ({
                                ...
                                created_atts: company.created_atts || null,
                            }))
    ...
    setSelectedCompany() {
        ...
        if (selectedCompanyId) {
            this.selectedState = this.companies.find((item) => item.unique_key === selectedCompanyId) || null;
        }
    
        if (!this.selectedState && this.companies.length > 0) {
            this.selectedState = this.companies[0];
            localStorage.setItem('selectedCompanyId', this.selectedState.unique_key);
        }
    }

    onCompanyChange(selectedState: any) {
        localStorage.setItem('selectedCompanyId', selectedState.unique_key);
