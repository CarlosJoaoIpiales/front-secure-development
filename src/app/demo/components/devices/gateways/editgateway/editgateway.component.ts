...
export class EditGatewayComponent implements OnInit, AfterViewInit {
    ...
    onSave(): void {
        ...
        if (this.selectedCompany) {
            this.gateway.unique_key_company = this.selectedCompany.unique_key;
        }
        ...
        if (this.valClass) {
            this.gateway.class_b = this.valClass;
        }
        if (this.selectedSubRed) {
            this.gateway.sub_red = this.selectedSubRed.name;
        } 
        if (this.valPKTFWD) {
    ...
    loadSelectedGateway(gatewayId: string): void {
        this.gatewayService.getGatewayById(gatewayId).subscribe(
            (data: Gateway) => {
                ...
                this.selectedCompany = this.companies.find(item => item.unique_key === this.gateway.unique_key_company);
                ...
                this.selectedSubRed = this.dropdownItemsSubred.find(item => item.name === this.gateway.sub_red);
                this.valClass = this.gateway.class_b;
    ...
    validateRequiredFields(): boolean {
        ...
        if (this.valCheck.includes('Mbus') && !this.gateway.mac_direction) {
            return false;
        }
        return true;
    }
