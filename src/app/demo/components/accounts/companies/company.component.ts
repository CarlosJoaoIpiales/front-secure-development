...
export class CompanyComponent implements OnInit {
    ...
    loadCompaniesByUser(): void {
        ...
        this.userCompanyService.getUserCompanies(userId).subscribe({
            next: (userCompanyRelations) => {
                const companiesIds = userCompanyRelations.map(relation => relation.company_unique_key);
    ...
    confirmDeleteSelected() {
        ...
        this.selectedCompanies.forEach(company => {
            this.deleteCompany(company.unique_key); 
            this.deleteUserCompany(company.unique_key);
            this.meterService.deleteMeterByCompanyID(company.unique_key);
    ...
    findIndexById(unique_key: string): number {
        let index = -1;
        for (let i = 0; i < this.companies.length; i++) {
            if (this.companies[i].unique_key === unique_key) {
                index = i;
                break;
            }
        }
    ...
    removeCompanyFromList(id: string) {
        this.companies = this.companies.filter(company => company.unique_key !== id);
    }
    ...
    openEdit(company: Company) {
        this.router.navigate([`/accounts/company/edit-company/${company.unique_key}`]);
    }
}
